{include file="frontend/components/header.tpl" pageTitleTranslated=$currentJournal->getLocalizedName()}

<div class="page_index_journal">
    {call_hook name="Templates::Index::journal"}

    {* === HERO SECTION === *}
    <section class="custom-hero-banner">
        <div class="container">
            <div class="hero-inner">

                <span class="hero-subtitle">
                    Scientific Journal
                </span>

                <h1 class="hero-title">
                    {$currentJournal->getLocalizedName()|escape}
                </h1>

                <p class="hero-tagline">
                    Mendorong inovasi melalui publikasi ilmiah yang berkualitas,
                    terbuka, dan terpercaya.
                </p>

                <div class="hero-btns">
                    <a href="{url page="about" op="submissions"}" class="btn-main">
                        Make a Submission
                    </a>
                    <a href="#homepageIssue" class="btn-outline-hero">
                        View Current Issue
                    </a>
                </div>

            </div>
        </div>
    </section>

    {* === MAIN CONTENT === *}
    <div class="container content-wrapper">
        <div class="row">
            <main class="col-lg-8 col-md-12">

               {if $activeTheme && $activeTheme->getOption('showDescriptionInJournalIndex')}
                <section class="homepage_about content-card">
                    <div class="card-header-custom">
                        <h2 class="section-title">
                            <svg class="icon-title" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path>
                                <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path>
                            </svg>
                            {translate key="about.aboutContext"}
                        </h2>
                    </div>
                    <div class="description-text">
                        {$currentContext->getLocalizedData('description') nofilter}
                    </div>
                </section>
                {/if}

                {if $issue}
                <section class="current_issue content-card" id="homepageIssue">
                    <div class="card-header-custom">
                        <h2 class="section-title">
                            <svg class="icon-title" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                                <polyline points="14 2 14 8 20 8"></polyline>
                                <line x1="16" y1="13" x2="8" y2="13"></line>
                                <line x1="16" y1="17" x2="8" y2="17"></line>
                                <polyline points="10 9 9 9 8 9"></polyline>
                            </svg>
                            {translate key="journal.currentIssue"}
                        </h2>
                        <span class="badge-new">Latest</span>
                    </div>

                    <div class="toc-wrapper">
                        {include file="frontend/objects/issue_toc.tpl" heading="h3"}
                    </div>
                </section>
                {/if}

            </main>

            <aside class="col-lg-4 col-md-12">
                <div class="custom-sidebar">
                    {call_hook name="Templates::Common::Sidebar"}
                </div>
            </aside>
        </div>
    </div>
</div>

{include file="frontend/components/footer.tpl"}